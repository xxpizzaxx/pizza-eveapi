...
class Eve(baseurl: String)(implicit ec: ExecutionContext) {
  ...
  def OwnerID(names: Seq[String]) = new ApiRequest[eve.OwnerID.Eveapi](baseurl, "Eve/OwnerID.xml.aspx", None, Map("names" -> names.mkString(",")))(eve.OwnerID.OwnerIDEveapiFormat).apply().map {_.map{r => new XMLApiResponse(r.currentTime.toDateTime, r.cachedUntil.toDateTime, r.result.rowset.row)}}
