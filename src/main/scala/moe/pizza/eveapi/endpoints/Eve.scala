...
class Eve(baseurl: String)(implicit c: Client) {
  ...
  def CharacterInfo(id: Long): Task[Either[XMLApiResponse[eve.CharacterInfo2.Result], XMLApiResponse[eve.CharacterInfo.Result]]] = new DualApiRequest[eve.CharacterInfo.Eveapi, eve.CharacterInfo2.Eveapi](baseurl, "Eve/CharacterInfo.xml.aspx", None, Map("characterID" -> id.toString))(eve.CharacterInfo2.CharacterInfo2EveapiFormat, eve.CharacterInfo.CharacterInfoEveapiFormat).apply().map { v =>
    v match {
